...
{
    public class Lz10_Compressor : BaseExtractor
    {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    var filesToProcess = allFiles.Where(IsNotLZ10File).ToArray();
                    ...
                    foreach (var filePath in filesToProcess)
                    {
                        ...
                        CompressLZ10File(filePath, compressedDir);
                    }
        ...
        private bool IsNotLZ10File(string filePath)
        {
            string extension = Path.GetExtension(filePath).ToLower();
            return extension != ".lz";
        }

        private bool CompressLZ10File(string inputPath, string outputDir)
        {
            try
            {
                ...
                string outputFilePath = Path.Combine(outputDir, relativePath + ".lz");
                ...
                {
                    lz10.Compress(originalData, compressedStream);
            ...
            {
                CompressionError?.Invoke(this, $"LZ10压缩错误:{ex.Message}");
                return false;
            }
        }

...
{
    public class Wav2brstm3_Converter : BaseExtractor
    {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                foreach (var wavFilePath in wavFiles)
                {
                    ...
                    try
                    {
                        string brstmFile = Path.Combine(fileDirectory, $"{fileName}.brstm");

                        if (File.Exists(brstmFile))
                            File.Delete(brstmFile);

                        bool conversionSuccess = await Task.Run(() =>
                            ConvertWavToBrstm(wavFilePath, brstmFile, cancellationToken));

                        if (conversionSuccess && File.Exists(brstmFile))
                        {
                            successCount++;
                            ConversionProgress?.Invoke(this, $"转换成功:{Path.GetFileName(brstmFile)}");
                            OnFileConverted(brstmFile);
                        }
                        else
                        {
        ...
        private bool ConvertWavToBrstm(string wavFilePath, string brstmFilePath, CancellationToken cancellationToken)
        {
            try
            {
                ...
                ConversionProgress?.Invoke(this, $"转换为BRSTM格式(DSP-ADPCM):{Path.GetFileName(brstmFilePath)}");

                var writer = new BrstmWriter();
                writer.Configuration = new BxstmConfiguration
                {
                    Codec = NwCodec.GcAdpcm,
                    SamplesPerInterleave = 8064,
                    SamplesPerSeekTableEntry = 8064,
                    LoopPointAlignment = 14,
                    TrackType = BrstmTrackType.Standard,
                    SeekTableType = BrstmSeekTableType.Standard,
                    RecalculateSeekTable = true,
                    RecalculateLoopContext = true
                };

                using (var brstmStream = File.Create(brstmFilePath))
                {
                    writer.WriteToStream(audioData, brstmStream, writer.Configuration);
                }

...
{
    public class Wav2xwma_Converter : BaseExtractor
    {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    ConversionStarted?.Invoke(this, $"开始转换WAV到xWMA,共{TotalFilesToConvert}个文件");
                    ...
                    foreach (var wavFilePath in wavFiles)
                    {
                        ...
                        string xwmaFilePath = Path.Combine(fileDirectory, $"{fileName}.xwma");

                        try
                        {
                            if (File.Exists(xwmaFilePath))
                                File.Delete(xwmaFilePath);

                            if (ConvertWavToXwma(wavFilePath, xwmaFilePath) && File.Exists(xwmaFilePath))
                            {
                                successCount++;
                                ConversionProgress?.Invoke(this, $"已转换:{fileName}.xwma");
                                OnFileConverted(xwmaFilePath);
                            }
                            else
                            {
        ...
        private bool ConvertWavToXwma(string wavFilePath, string xwmaFilePath)
        {
            try
            {
                var process = new Process
                {
                    StartInfo = new ProcessStartInfo
                    {
                        ...
                        Arguments = $"\"{wavFilePath}\" \"{xwmaFilePath}\"",
                ...
                return File.Exists(xwmaFilePath);
            }
            catch (Exception ex)
            {

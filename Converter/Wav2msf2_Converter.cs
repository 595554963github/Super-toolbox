...
{
    public class Wav2msf2_Converter : BaseExtractor
    {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            ConversionStarted?.Invoke(this, $"开始处理目录:{directoryPath}(小端序MSF)");
                ...
                foreach (var wavFilePath in wavFiles)
                {
                    ...
                    try
                    {
                        ...
                        bool conversionSuccess = await Task.Run(() =>
                            ConvertWavToMsfLittleEndian(wavFilePath, msfFile, cancellationToken));
        ...
        private bool ConvertWavToMsfLittleEndian(string wavFilePath, string msfFilePath, CancellationToken cancellationToken)
        {
            try
            {
                ...
                ConversionProgress?.Invoke(this, $"转换为MSF格式(小端序):{Path.GetFileName(msfFilePath)}");

                byte[] msfData = CreateMsfLittleEndian(pcmFormat);
        ...
        private byte[] CreateMsfLittleEndian(Pcm16Format pcmFormat)
        {

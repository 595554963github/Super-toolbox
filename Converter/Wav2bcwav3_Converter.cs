...
{
    public class Wav2bcwav3_Converter : BaseExtractor
    {
        private const int CWAV_ENCODING_DSP_ADPCM = 2;
        ...
        static Wav2bcwav3_Converter()
        {
            try
            {
                _tempDllPath = LoadEmbeddedDll("embedded.bcwavtool.dll", "bcwavtool_dsp.dll");
                _dllLoaded = true;
            }
            catch (Exception ex)
            {
        ...
        [DllImport("bcwavtool_dsp.dll", CallingConvention = CallingConvention.Cdecl)]
            ...
            try
            {
                foreach (var wavFilePath in wavFiles)
                {
                    ...
                    try
                    {
                        ...
                        {
                            successCount++;
                            ConversionProgress?.Invoke(this, $"转换成功:{Path.GetFileName(bcwavFile)}(DSP-ADPCM)");
        ...
        private bool ConvertWavToBcwav(string wavFilePath, string bcwavFilePath, CancellationToken cancellationToken)
        {
            try
            {
                ...
                int result = BCWAV_Encode(wavFilePath, bcwavFilePath, CWAV_ENCODING_DSP_ADPCM, false, 0, 0);

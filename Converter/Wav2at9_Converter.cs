...
{
    public class Wav2at9_Converter : BaseExtractor
    {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    foreach (var wavFilePath in wavFiles)
                    {
                        ...
                        string at9FilePath = Path.Combine(fileDirectory, $"{fileName}.at9");

                        try
                        {
                            if (File.Exists(at9FilePath))
                                File.Delete(at9FilePath);

                            if (ConvertWavToAt9(wavFilePath, at9FilePath) && File.Exists(at9FilePath))
                            {
                                successCount++;
                                ConversionProgress?.Invoke(this, $"已转换:{fileName}.at9");
                                OnFileConverted(at9FilePath);
                            }
                            else
                            {
        ...
        private bool ConvertWavToAt9(string wavFilePath, string at9FilePath)
        {
            try
            {
                var process = new Process
                {
                    StartInfo = new ProcessStartInfo
                    {
                        ...
                        Arguments = $"-e \"{wavFilePath}\" \"{at9FilePath}\"",
                ...
                return File.Exists(at9FilePath);
            }
            catch (Exception ex)
            {

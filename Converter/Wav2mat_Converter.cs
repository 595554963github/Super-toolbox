...
{
    public class Wav2mat_Converter : BaseExtractor
    {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                foreach (var wavFilePath in wavFiles)
                {
                    ...
                    string matFilePath = Path.Combine(fileDirectory, $"{fileName}.mat");

                    try
                    {
                        if (File.Exists(matFilePath)) File.Delete(matFilePath);

                        bool conversionSuccess = await ConvertWavToMat(wavFilePath, matFilePath, cancellationToken);

                        if (conversionSuccess && File.Exists(matFilePath))
                        {
                            successCount++;
                            ConversionProgress?.Invoke(this, $"转换成功:{Path.GetFileName(matFilePath)}");
                            OnFileConverted(matFilePath);
                        }
                        else
                        {
        ...
        private async Task<bool> ConvertWavToMat(string wavFilePath, string matFilePath, CancellationToken cancellationToken)
        {
            try
            {
                ...
                using (var process = Process.Start(processStartInfo))
                {
                    ...
                    string expectedOutputPath = Path.Combine(Path.GetDirectoryName(wavFilePath) ?? string.Empty, Path.GetFileNameWithoutExtension(wavFilePath) + ".mat");

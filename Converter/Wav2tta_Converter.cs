...
{
    public class Wav2tta_Converter : BaseExtractor
    {
        ...
        static Wav2tta_Converter()
        {
            try
            {
                _tempExePath = ExtractEmbeddedExe("embedded.ttaenc.exe", "ttaenc1.exe");
                _exeExtracted = true;
            }
            catch (Exception ex)
            {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                foreach (var wavFilePath in wavFiles)
                {
                    ...
                    string ttaFile = Path.Combine(fileDirectory, $"{fileName}.tta");

                    try
                    {
                        if (File.Exists(ttaFile))
                        {
                            File.Delete(ttaFile);
                        }

                        bool conversionSuccess = await Task.Run(() =>
                            ConvertWavToTta(wavFilePath, ttaFile, cancellationToken));

                        if (conversionSuccess && File.Exists(ttaFile))
                        {
                            successCount++;
                            ConversionProgress?.Invoke(this, $"转换成功:{Path.GetFileName(ttaFile)}");
                            OnFileConverted(ttaFile);
                        }
                        else
                        {
        ...
        private bool ConvertWavToTta(string wavFilePath, string ttaFilePath, CancellationToken cancellationToken)
        {
            try
            {
                string arguments = $"-e \"{wavFilePath}\" \"{ttaFilePath}\"";

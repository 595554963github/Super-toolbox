...
{
    public class Wav2binka2_Converter : BaseExtractor
    {
        ...
        static Wav2binka2_Converter()
        {
            _tempExePath = LoadEmbeddedExe("embedded.binka_encode.exe", "binka_encode.exe");
        }
            ...
            try
            {
                foreach (var wavFilePath in wavFiles)
                {
                    ...
                    try
                    {
                        ...
                        bool conversionSuccess = await ConvertWavToBinka(wavFilePath, binkaFilePath, cancellationToken);
        ...
        private async Task<bool> ConvertWavToBinka(string wavFilePath, string binkaFilePath, CancellationToken cancellationToken)
        {
            try
            {
                var processStartInfo = new ProcessStartInfo
                {
                    ...
                    Arguments = $"\"{wavFilePath}\" \"{binkaFilePath}\"",
                ...
                using (var process = Process.Start(processStartInfo))
                {
                    ...
                    return process.ExitCode == 0 && File.Exists(binkaFilePath);
                }
            }
            catch (Exception ex)
            {

...
namespace super_toolbox
{
    public class Bfwav2wav_Converter : BaseExtractor
    {
        ...
        private bool ConvertBfwavToWav(string bfwavFilePath, string wavFilePath, CancellationToken cancellationToken)
        {
            try
            {
                ConversionProgress?.Invoke(this, $"读取BFWAV文件:{Path.GetFileName(bfwavFilePath)}");

                var bfwavReader = new BCFstmReader();
                AudioData audioData;

                using (var bfwavStream = File.OpenRead(bfwavFilePath))
                {
                    audioData = bfwavReader.Read(bfwavStream);
                }

                if (audioData == null)
                {
                    throw new InvalidOperationException("无法读取BFWAV音频数据");
                }

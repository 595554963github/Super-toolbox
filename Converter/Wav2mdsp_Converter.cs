...
{
    public class Wav2mdsp_Converter : BaseExtractor
    {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                foreach (var wavFilePath in wavFiles)
                {
                    ...
                    try
                    {
                        string mdspFile = Path.Combine(fileDirectory, $"{fileName}.mdsp");

                        if (File.Exists(mdspFile))
                            File.Delete(mdspFile);

                        bool conversionSuccess = await Task.Run(() =>
                            ConvertWavToMdsp(wavFilePath, mdspFile, cancellationToken));

                        if (conversionSuccess && File.Exists(mdspFile))
                        {
                            successCount++;
                            ConversionProgress?.Invoke(this, $"转换成功:{Path.GetFileName(mdspFile)}");
                            OnFileConverted(mdspFile);
                        }
                        else
                        {
        ...
        private bool ConvertWavToMdsp(string wavFilePath, string mdspFilePath, CancellationToken cancellationToken)
        {
            try
            {
                ...
                ConversionProgress?.Invoke(this, $"转换为MDSP格式:{Path.GetFileName(mdspFilePath)}");
                ...
                using (var mdspStream = File.Create(mdspFilePath))
                {
                    dspWriter.WriteToStream(audioData, mdspStream, dspConfig);
                }

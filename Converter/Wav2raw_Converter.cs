...
{
    public class Wav2raw_Converter : BaseExtractor
    {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                foreach (var wavFilePath in wavFiles)
                {
                    ...
                    string rawFilePath = Path.Combine(fileDirectory, $"{fileName}.raw");

                    try
                    {
                        if (File.Exists(rawFilePath)) File.Delete(rawFilePath);

                        bool conversionSuccess = await ConvertWavToRaw(wavFilePath, rawFilePath, cancellationToken);

                        if (conversionSuccess && File.Exists(rawFilePath))
                        {
                            successCount++;
                            ConversionProgress?.Invoke(this, $"转换成功:{Path.GetFileName(rawFilePath)}");
                            OnFileConverted(rawFilePath);
                        }
                        else
                        {
        ...
        private async Task<bool> ConvertWavToRaw(string wavFilePath, string rawFilePath, CancellationToken cancellationToken)
        {
            try
            {
                ...
                using (var process = Process.Start(processStartInfo))
                {
                    ...
                    string expectedOutputPath = Path.Combine(Path.GetDirectoryName(wavFilePath) ?? string.Empty, Path.GetFileNameWithoutExtension(wavFilePath) + ".raw");

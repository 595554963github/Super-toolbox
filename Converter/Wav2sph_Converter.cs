...
{
    public class Wav2sph_Converter : BaseExtractor
    {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                foreach (var wavFilePath in wavFiles)
                {
                    ...
                    string sphFilePath = Path.Combine(fileDirectory, $"{fileName}.sph");

                    try
                    {
                        if (File.Exists(sphFilePath)) File.Delete(sphFilePath);

                        bool conversionSuccess = await ConvertWavToSph(wavFilePath, sphFilePath, cancellationToken);

                        if (conversionSuccess && File.Exists(sphFilePath))
                        {
                            successCount++;
                            ConversionProgress?.Invoke(this, $"转换成功:{Path.GetFileName(sphFilePath)}");
                            OnFileConverted(sphFilePath);
                        }
                        else
                        {
        ...
        private async Task<bool> ConvertWavToSph(string wavFilePath, string sphFilePath, CancellationToken cancellationToken)
        {
            try
            {
                ...
                using (var process = Process.Start(processStartInfo))
                {
                    ...
                    string expectedOutputPath = Path.Combine(Path.GetDirectoryName(wavFilePath) ?? string.Empty, Path.GetFileNameWithoutExtension(wavFilePath) + ".sph");

...
{
    public class Wav2aiff_Converter : BaseExtractor
    {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                foreach (var wavFilePath in wavFiles)
                {
                    ...
                    string aiffFile = Path.Combine(fileDirectory, $"{fileName}.aiff");
                    ...
                    try
                    {
                        ...
                        bool conversionSuccess = await Task.Run(() =>
                            ConvertWavToAiff(wavFilePath, aiffFile, cancellationToken));

                        if (conversionSuccess && File.Exists(aiffFile))
                        {
                            successCount++;
                            ConversionProgress?.Invoke(this, $"转换成功:{Path.GetFileName(aiffFile)}");
                            OnFileConverted(aiffFile);
                        }
                        else
                        {
        ...
        private bool ConvertWavToAiff(string wavFilePath, string aiffFilePath, CancellationToken cancellationToken)
        {
            try
            {
                ...
                using (var writer = new BinaryWriter(ms))
                {
                    ...
                    File.WriteAllBytes(aiffFilePath, ms.ToArray());
                }

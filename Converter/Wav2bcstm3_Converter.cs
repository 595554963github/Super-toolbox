...
{
    public class Wav2bcstm3_Converter : BaseExtractor
    {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                foreach (var wavFilePath in wavFiles)
                {
                    ...
                    try
                    {
                        string bcstmFile = Path.Combine(fileDirectory, $"{fileName}.bcstm");

                        if (File.Exists(bcstmFile))
                            File.Delete(bcstmFile);

                        bool conversionSuccess = await Task.Run(() =>
                            ConvertWavToBcstm(wavFilePath, bcstmFile, cancellationToken));

                        if (conversionSuccess && File.Exists(bcstmFile))
                        {
                            successCount++;
                            ConversionProgress?.Invoke(this, $"转换成功:{Path.GetFileName(bcstmFile)}");
                            OnFileConverted(bcstmFile);
                        }
                        else
                        {
        ...
        private bool ConvertWavToBcstm(string wavFilePath, string bcstmFilePath, CancellationToken cancellationToken)
        {
            try
            {
                ...
                ConversionProgress?.Invoke(this, $"转换为BCSTM格式(DSP-ADPCM):{Path.GetFileName(bcstmFilePath)}");
                ...
                using (var bcstmStream = File.Create(bcstmFilePath))
                {
                    writer.WriteToStream(audioData, bcstmStream, writer.Configuration);
                }

...
namespace super_toolbox
{
    public class AfsRepacker : BaseExtractor
    {
        ...
        static AfsRepacker()
        {
            ...
            _tempExePath = Path.Combine(tempDir, "afs_util.exe");
            if (!File.Exists(_tempExePath))
            {
                using (var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("embedded.afs_util.exe"))
                {
                    if (stream == null)
                        throw new FileNotFoundException("嵌入的afs_util.exe资源未找到");
        ...
        public async Task RepackAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    foreach (var folderPath in directories)
                    {
                        ...
                        {
                            OnExtractionFailed($"AFS打包失败: {folderName} - 错误: {error}");
                            continue;
                        }

                        string expectedAfsFile = Path.Combine(parentDir, folderName + ".afs");
                        if (File.Exists(expectedAfsFile))
                        {
                            OnFileExtracted(expectedAfsFile);
                        }
                        else
                        {
                            OnExtractionFailed($"AFS打包失败: 未找到输出文件 {expectedAfsFile}");
                        }
                    }
            ...
            {
                OnExtractionFailed($"AFS处理失败: {ex.Message}");
            }
        }

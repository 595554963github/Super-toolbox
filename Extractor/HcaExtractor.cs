namespace super_toolbox
{
    public class HcaExtractor : BaseExtractor
    {
            ...
            try
            {
                foreach (var filePath in filePaths)
                {
                    ...
                    try
                    {
                        ...
                        int count = await ExtractHcaTypeAsync(content, filePath, extractedDir, START_SEQ_1, HCA_BLOCK_MARKER, extractedFiles, cancellationToken);
                        count += await ExtractHcaTypeAsync(content, filePath, extractedDir, START_SEQ_2, null, extractedFiles, cancellationToken);
                        if (count > 0)
                        {
        ...
        private async Task<int> ExtractHcaTypeAsync(byte[] content, string filePath, string extractedDir,
            byte[] startSequence, byte[]? blockMarker, List<string> extractedFiles, CancellationToken cancellationToken)
        {
            int count = 0;
            int index = 0;

    ...
    public class RIFF_RIFX_Sound_Extractor : BaseExtractor
    {
                ...
                ["wav"] = new List<byte[]>
        {
            new byte[] { 0x10, 0x00, 0x00, 0x00, 0x01, 0x00 }
                ...
                ["wavex"] = new List<byte[]>
        {
            new byte[] { 0x28, 0x00, 0x00, 0x00, 0xFE, 0xFF }
        }
            };
            ...
            byte[] atracPattern = new byte[] { 0x34, 0x00, 0x00, 0x00, 0xFE, 0xFF };
            if (magic.SequenceEqual(atracPattern))
            {
                if (buffer.Length - position >= 0x2C + 16)
                {
                    int guidOffset = position + 0x2C;

                    byte[] atrac3pGuid = new byte[]
                    {
                0xBF, 0xAA, 0x23, 0xE9, 0x58, 0xCB, 0x71, 0x44,
                0xA1, 0x19, 0xFF, 0xFA, 0x01, 0xE4, 0xCE, 0x62
                    };

                    byte[] atrac9Guid = new byte[]
                    {
                0xD2, 0x42, 0xE1, 0x47, 0xBA, 0x36, 0x8D, 0x4D,
                0x88, 0xFC, 0x61, 0x65, 0x4F, 0x8C, 0x83, 0x6C
                    };

                    Span<byte> guidSpan = new Span<byte>(buffer, guidOffset, 16);

                    if (guidSpan.SequenceEqual(atrac3pGuid))
                        return "at3";

                    if (guidSpan.SequenceEqual(atrac9Guid))
                        return "at9";
                }

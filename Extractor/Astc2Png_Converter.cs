...
namespace super_toolbox
{
    public class Astc2Png_Converter : BaseExtractor
    {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            TotalFilesToExtract = Directory.GetFiles(directoryPath, "*.astc", SearchOption.AllDirectories).Length;
                ...
                foreach (var astcFilePath in astcFiles)
                {
                    ...
                    try
                    {
                        ...
                        if (conversionSuccess && File.Exists(pngFilePath))
                        {
                            ...
                            OnFileExtracted(pngFilePath);
                        }
                        else
                        {
                            ConversionError?.Invoke(this, $"{fileName}.astc转换失败");
                            OnExtractionFailed($"{fileName}.astc转换失败");
                        }
                    }
                    catch (Exception ex)
                    {
                        ConversionError?.Invoke(this, $"转换异常:{ex.Message}");
                        OnExtractionFailed($"{fileName}.astc 处理错误:{ex.Message}");
                    }
                }

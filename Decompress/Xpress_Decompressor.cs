...
{
    public class Xpress_Decompressor : BaseExtractor
    {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            var filesToProcess = allFiles.Where(f => f.EndsWith(".xprs", StringComparison.OrdinalIgnoreCase)).ToArray();

            if (filesToProcess.Length == 0)
            {
                DecompressionError?.Invoke(this, "未找到有效的Xpress压缩文件");
                OnDecompressionFailed("未找到有效的Xpress压缩文件");
                return;
            }
                ...
                await Task.Run(() =>
                {
                    ...
                    foreach (var filePath in filesToProcess)
                    {
                        ...
                        try
                        {
                            ...
                            var decompressedData = DecompressXpress(compressedData);
        ...
        private byte[] DecompressXpress(byte[] compressedData)
        {
            IntPtr decompressor = IntPtr.Zero;
            try
            {
                if (!CreateDecompressor(COMPRESS_ALGORITHM_XPRESS, IntPtr.Zero, out decompressor))

...
{
    public class Mszip_Decompressor : BaseExtractor
    {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            var filesToProcess = allFiles.Where(f => f.EndsWith(".mszip", StringComparison.OrdinalIgnoreCase)).ToArray();

            if (filesToProcess.Length == 0)
            {
                DecompressionError?.Invoke(this, "未找到有效的MSZip压缩文件");
                OnDecompressionFailed("未找到有效的MSZip压缩文件");
                return;
            }
                ...
                await Task.Run(() =>
                {
                    ...
                    foreach (var filePath in filesToProcess)
                    {
                        ...
                        try
                        {
                            ...
                            var decompressedData = DecompressMSZip(compressedData);
        ...
        private byte[] DecompressMSZip(byte[] compressedData)
        {
            IntPtr decompressor = IntPtr.Zero;
            try
            {
                if (!CreateDecompressor(COMPRESS_ALGORITHM_MSZIP, IntPtr.Zero, out decompressor))

...
{
    public class Allz_Decompressor : BaseExtractor
    {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    var filesToProcess = allFiles.Where(IsAllzFile).ToArray();
                    if (filesToProcess.Length == 0)
                    {
                        DecompressionError?.Invoke(this, "未找到有效的allz压缩文件");
                        OnDecompressionFailed("未找到有效的allz压缩文件");
                        return;
                    }
                    ...
                    foreach (var filePath in filesToProcess)
                    {
                        ...
                        DecompressAllzFile(filePath, decompressedDir);
                    }
        ...
        private bool IsAllzFile(string filePath)
        {
            string extension = Path.GetExtension(filePath).ToLower();
            return extension == ".allz";
        }

        private bool DecompressAllzFile(string inputPath, string outputDir)
        {
            try
            {
                ...
                string relativePathWithoutExt = relativePath.Substring(0, relativePath.Length - 5);
                ...
                {
                    allz.Decompress(compressedStream, decompressedStream);
            ...
            {
                DecompressionError?.Invoke(this, $"allz解压错误:{ex.Message}");
                return false;
            }
        }

...
{
    public class Lz10_Decompressor : BaseExtractor
    {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    var filesToProcess = allFiles.Where(IsLZ10File).ToArray();
                    if (filesToProcess.Length == 0)
                    {
                        DecompressionError?.Invoke(this, "未找到有效的LZ10压缩文件");
                        OnDecompressionFailed("未找到有效的LZ10压缩文件");
                        return;
                    }
                    ...
                    foreach (var filePath in filesToProcess)
                    {
                        ...
                        DecompressLZ10File(filePath, decompressedDir);
                    }
        ...
        private bool IsLZ10File(string filePath)
        {
            string extension = Path.GetExtension(filePath).ToLower();
            return extension == ".lz";
        }

        private bool DecompressLZ10File(string inputPath, string outputDir)
        {
            try
            {
                ...
                string relativePathWithoutExt = relativePath.Substring(0, relativePath.Length - 3);
                ...
                {
                    lz10.Decompress(compressedStream, decompressedStream);
            ...
            {
                DecompressionError?.Invoke(this, $"LZ10解压错误:{ex.Message}");
                return false;
            }
        }

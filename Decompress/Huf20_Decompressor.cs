...
{
    public class Huf20_Decompressor : BaseExtractor
    {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    var filesToProcess = allFiles.Where(IsHuf20File).ToArray();
                    if (filesToProcess.Length == 0)
                    {
                        DecompressionError?.Invoke(this, "未找到有效的huf20压缩文件");
                        OnDecompressionFailed("未找到有效的huf20压缩文件");
                        return;
                    }
                    ...
                    foreach (var filePath in filesToProcess)
                    {
                        ...
                        DecompressHUF20File(filePath, decompressedDir);
                    }
        ...
        private bool IsHuf20File(string filePath)
        {
            string extension = Path.GetExtension(filePath).ToLower();
            return extension == ".huf";
        }

        private bool DecompressHUF20File(string inputPath, string outputDir)
        {
            try
            {
                ...
                string relativePathWithoutExt = relativePath.Substring(0, relativePath.Length - 4);
                ...
                {
                    huf20.Decompress(compressedStream, decompressedStream);
            ...
            {
                DecompressionError?.Invoke(this, $"huf20解压错误:{ex.Message}");
                return false;
            }
        }

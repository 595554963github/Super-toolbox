...
namespace super_toolbox
{
    public class Lzx_Compressor : BaseExtractor
    {
        ...
        private static extern int Compress(byte[] src, uint src_size, byte[] dest, out uint compressed_size);
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            var filesToCompress = Directory.GetFiles(directoryPath, "*.*", SearchOption.AllDirectories)
                .Where(file => !file.EndsWith(".lzx", StringComparison.OrdinalIgnoreCase))
                .ToArray();
            ...
            try
            {
                await Task.Run(() =>
                {
                    foreach (var file in Directory.GetFiles(compressedDir, "*.lzx", SearchOption.AllDirectories))
                    {
                        File.Delete(file);
                    }
                    ...
                    foreach (var filePath in filesToCompress)
                    {
                        ...
                        string outputPath = Path.Combine(compressedDir, relativePath + ".lzx");
                        ...
                        if (CompressLzxFile(filePath, outputPath))
                        {
        ...
        private bool CompressLzxFile(string inputPath, string outputPath)
        {
            try
            {
                byte[] inputData = File.ReadAllBytes(inputPath);
                byte[] outputData = new byte[inputData.Length + 6144];

                int result = Compress(inputData, (uint)inputData.Length, outputData, out uint compressedSize);

                if (result == 0 && compressedSize > 0 && compressedSize <= outputData.Length)
                {
                ...
                {
                    CompressionError?.Invoke(this, $"压缩失败({Path.GetFileName(inputPath)}):返回码={result},大小={compressedSize}");
                    return false;
                }
            }
            catch (Exception ex)

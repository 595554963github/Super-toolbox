...
{
    public class Xpress_Compressor : BaseExtractor
    {
        private const uint COMPRESS_ALGORITHM_XPRESS = 3;
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            var filesToCompress = Directory.GetFiles(directoryPath, "*.*", SearchOption.AllDirectories)
                .Where(file => !file.EndsWith(".xprs", StringComparison.OrdinalIgnoreCase))
                .ToArray();
            ...
            try
            {
                await Task.Run(() =>
                {
                    foreach (var file in Directory.GetFiles(compressedDir, "*.xprs", SearchOption.AllDirectories))
                    {
                        File.Delete(file);
                    }
                    ...
                    foreach (var filePath in filesToCompress)
                    {
                        ...
                        string outputPath = Path.Combine(compressedDir, relativePath + ".xprs");
                        ...
                        if (CompressXpressFile(filePath, outputPath))
                        {
        ...
        private bool CompressXpressFile(string inputPath, string outputPath)
        {
            try
            {
                byte[] inputData = File.ReadAllBytes(inputPath);
                byte[] compressedData = CompressXpress(inputData);
        ...
        private byte[] CompressXpress(byte[] data)
        {
            IntPtr compressor = IntPtr.Zero;
            try
            {
                if (!CreateCompressor(COMPRESS_ALGORITHM_XPRESS, IntPtr.Zero, out compressor))

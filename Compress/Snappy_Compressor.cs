...
{
    public class Snappy_Compressor : BaseExtractor
    {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    foreach (var file in Directory.GetFiles(compressedDir, "*.sz", SearchOption.AllDirectories))
                    {
                        File.Delete(file);
                    }
                    ...
                    foreach (var filePath in filesToCompress)
                    {
                        ...
                        string outputPath = Path.Combine(compressedDir, relativePath + ".sz");
                        ...
                        try
                        {
                            if (CompressFileWithSnappy(filePath, outputPath))
                            {
                            ...
                            {
                                CompressionError?.Invoke(this, $"压缩文件失败:{filePath}");
                                OnCompressionFailed($"压缩文件失败:{filePath}");
                            }
                        }
                        catch (Exception ex)
                        {
        ...
        private bool CompressFileWithSnappy(string inputPath, string outputPath)
        {
            try
            {
                byte[] inputData = File.ReadAllBytes(inputPath);
                byte[] compressedData = Snappy.Encode(inputData);
            ...
            {
                CompressionError?.Invoke(this, $"Snappy压缩错误:{ex.Message}");
                return false;
            }
        }

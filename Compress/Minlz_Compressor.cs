...
namespace super_toolbox
{
    public class Minlz_Compressor : BaseExtractor
    {
        ...
        static Minlz_Compressor()
        {
            string tempDir = Path.Combine(Path.GetTempPath(), "supertoolbox_temp");
            Directory.CreateDirectory(tempDir);
            _tempExePath = Path.Combine(tempDir, "minlz.exe");
            if (!File.Exists(_tempExePath))
            {
                ...
                if (string.IsNullOrEmpty(resourceName))
                    throw new FileNotFoundException("嵌入的MinLZ压缩工具资源未找到");
                using (var stream = assembly.GetManifestResourceStream(resourceName))
                {
                    if (stream == null)
                        throw new FileNotFoundException("无法读取嵌入的MinLZ压缩工具资源");
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    foreach (var file in Directory.GetFiles(compressedDir, "*.mz", SearchOption.AllDirectories))
                    {
                        File.Delete(file);
                    }
                    ...
                    foreach (var filePath in filesToCompress)
                    {
                        ...
                        string outputPath = Path.Combine(compressedDir, relativePath + ".mz");
                        ...
                        {
                            string actualOutputPath = filePath + ".mz";

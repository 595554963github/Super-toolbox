...
namespace super_toolbox
{
    public class Lz4c_Compressor : BaseExtractor
    {
        ...
        static Lz4c_Compressor()
        {
            string tempDir = Path.Combine(Path.GetTempPath(), "supertoolbox_temp");
            Directory.CreateDirectory(tempDir);
            _tempExePath = Path.Combine(tempDir, "lz4c.exe");
            if (!File.Exists(_tempExePath))
            {
                ...
                if (string.IsNullOrEmpty(resourceName))
                    throw new FileNotFoundException("嵌入的LZ4压缩工具资源未找到");
                using (var stream = assembly.GetManifestResourceStream(resourceName))
                {
                    if (stream == null)
                        throw new FileNotFoundException("无法读取嵌入的LZ4压缩工具资源");
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    foreach (var file in Directory.GetFiles(compressedDir, "*.lz4", SearchOption.AllDirectories))
                    {
                        File.Delete(file);
                    }
                    ...
                    foreach (var filePath in filesToCompress)
                    {
                        ...
                        string outputPath = Path.Combine(compressedDir, relativePath + ".lz4");
                        ...
                        {
                            string actualOutputPath = filePath + ".lz4";
                            if (File.Exists(actualOutputPath) && new FileInfo(actualOutputPath).Length > 0)
                            {
                            ...
                            {
                                CompressionError?.Invoke(this, $"压缩成功但输出文件异常:{actualOutputPath}");
                                OnCompressionFailed($"压缩成功但输出文件异常:{actualOutputPath}");
                            }
                        }
                        else
                        {

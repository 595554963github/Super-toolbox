            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    foreach (var filePath in filesToCompress)
                    {
                        ...
                        string outputPath = Path.Combine(compressedDir, relativePath);
                        ...
                        try
                        {
                            ...
                            byte[] compressedData = Prs.Compress(originalData, 0x1FFF);
                            ...
                            if (File.Exists(outputPath) && new FileInfo(outputPath).Length > 0)
                            {
                                ...
                                CompressionProgress?.Invoke(this,
                                    $"已压缩:{Path.GetFileName(outputPath)} " +
                                    $"(原始:{originalSize}字节, 压缩后:{compressedSize}字节, 压缩率:{ratio:F1}%)");
                        ...
                        {
                            CompressionError?.Invoke(this, $"压缩失败({filePath}): {ex.Message}");
                            OnCompressionFailed($"压缩失败({filePath}): {ex.Message}");
                        }
                    }

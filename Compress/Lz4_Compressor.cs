...
{
    public class Lz4_Compressor : BaseExtractor
    {
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    foreach (var filePath in filesToCompress)
                    {
                        ...
                        string outputPath = Path.Combine(compressedDir, fileName + ".lz4");

                        CompressFileWithLz4(filePath, outputPath);
            ...
            {
                OnExtractionFailed($"LZ4压缩失败: {ex.Message}");
            }
        }

        private void CompressFileWithLz4(string inputPath, string outputPath)
        {

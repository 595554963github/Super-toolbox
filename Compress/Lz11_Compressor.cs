...
namespace super_toolbox
{
    public class Lz11_Compressor : BaseExtractor
    {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    ...
                    var filesToProcess = allFiles.Where(IsNotLZ11File).ToArray();
                    ...
                    foreach (var filePath in filesToProcess)
                    {
                        ...
                        CompressLZ11File(filePath, compressedDir);
                    }
        ...
        private bool IsNotLZ11File(string filePath)
        {
            string extension = Path.GetExtension(filePath).ToLower();
            return extension != ".lz11" && extension != ".lz";
        }

        private bool CompressLZ11File(string inputPath, string outputDir)
        {
            try
            {
                ...
                string outputFilePath = Path.Combine(outputDir, relativePath + ".lz11");
                ...
                {
                    lz11.Compress(originalData, compressedStream);
            ...
            {
                CompressionError?.Invoke(this, $"LZ11压缩错误:{ex.Message}");
                return false;
            }
        }

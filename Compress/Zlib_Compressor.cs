...
{
    public class Zlib_Compressor : BaseExtractor
    {
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                await Task.Run(() =>
                {
                    foreach (var file in Directory.GetFiles(compressedDir, "*.zlib"))
                    {
                        File.Delete(file);
                    }
                    ...
                    foreach (var filePath in filesToCompress)
                    {
                        ...
                        string outputPath = Path.Combine(compressedDir, fileName + ".zlib");

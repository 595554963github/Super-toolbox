...
{
    public class XpressHuff_Compressor : BaseExtractor
    {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            var filesToCompress = Directory.GetFiles(directoryPath, "*.*", SearchOption.AllDirectories)
                .Where(file => !file.EndsWith(".xph", StringComparison.OrdinalIgnoreCase))
                .ToArray();
            ...
            try
            {
                await Task.Run(() =>
                {
                    foreach (var file in Directory.GetFiles(compressedDir, "*.xph", SearchOption.AllDirectories))
                    {
                        File.Delete(file);
                    }
                    ...
                    foreach (var filePath in filesToCompress)
                    {
                        ...
                        string outputPath = Path.Combine(compressedDir, relativePath + ".xph");
                        ...
                        if (CompressXpressHuffFile(filePath, outputPath))
                        {
        ...
        private bool CompressXpressHuffFile(string inputPath, string outputPath)
        {
            try
            {
                byte[] inputData = File.ReadAllBytes(inputPath);
                byte[] compressedData = CompressXpressHuff(inputData);
        ...
        private byte[] CompressXpressHuff(byte[] data)
        {
            IntPtr compressor = IntPtr.Zero;
            try
            {
                if (!CreateCompressor(COMPRESS_ALGORITHM_XPRESS_HUFF, IntPtr.Zero, out compressor))

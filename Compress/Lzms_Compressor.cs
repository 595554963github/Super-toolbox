...
{
    public class Lzms_Compressor : BaseExtractor
    {
        ...
        public override async Task ExtractAsync(string directoryPath, CancellationToken cancellationToken = default)
        {
            ...
            var filesToCompress = Directory.GetFiles(directoryPath, "*.*", SearchOption.AllDirectories)
                .Where(file => !file.EndsWith(".lzms", StringComparison.OrdinalIgnoreCase))
                .ToArray();
            ...
            try
            {
                await Task.Run(() =>
                {
                    foreach (var file in Directory.GetFiles(compressedDir, "*.lzms", SearchOption.AllDirectories))
                    {
                        File.Delete(file);
                    }
                    ...
                    foreach (var filePath in filesToCompress)
                    {
                        ...
                        string outputPath = Path.Combine(compressedDir, relativePath + ".lzms");
                        ...
                        if (CompressLzmsFile(filePath, outputPath))
                        {
        ...
        private bool CompressLzmsFile(string inputPath, string outputPath)
        {
            try
            {
                byte[] inputData = File.ReadAllBytes(inputPath);
                byte[] compressedData = CompressLzms(inputData);
        ...
        private byte[] CompressLzms(byte[] data)
        {
            IntPtr compressor = IntPtr.Zero;
            try
            {
                if (!CreateCompressor(COMPRESS_ALGORITHM_LZMS, IntPtr.Zero, out compressor))
